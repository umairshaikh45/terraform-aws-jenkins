...
/*Redeclaring provider here because terraform by default assume provider from harshicorp/docker */
terraform {
  required_providers {
    docker = {
...
resource "aws_ecs_task_definition" "Job" {
  family = "jenkins"
  container_definitions = templatefile("modules/jenkins/templates/jenkins.json.tpl", {
...
resource "aws_launch_template" "launch_template" {
  ...
  user_data     = base64encode("${templatefile("modules/jenkins/script/instance_setup.sh", { cluster_name = "${aws_ecs_cluster.jenkins.name}"})}")
  network_interfaces {
